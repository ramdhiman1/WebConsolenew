...
public class Production_Server_WWW3 extends Base_Page {
	...
	@Test(priority = 1, groups = { "smoke",
			"Regression" }, description = "Log in to the Cloud Console on 'www3' to verify machine status, install products, and confirm successful installation.")
	public void loginToWww3CloudConsole() throws InterruptedException, IOException {
		...
		getDriver().get(p.getProperty("www3computerspage"));
		iw2.SearchMachine();
		...
		driver.get().get(p.getProperty("www3taskstatuspage"));
		iw2.checkfirestrow1();
		logToReport(Status.INFO, "Checked task status");

		driver.get().get(p.getProperty("www3applicationpage"));
		...
		logToReport(Status.INFO, "Signed out from www3 Cloud Console");
	}
}
